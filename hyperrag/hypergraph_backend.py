...
class HypergraphDriver(Protocol):
    ...
    def load_or_create(self, database_name: str):
        """Load a hypergraph identified by ``database_name`` or create a new one."""
...
class HypergraphDBDriver:
    ...
    def load_or_create(self, database_name: str):
        # The hypergraph-db backend expects a local storage file path as the database name.
        return self._impl(storage_file=database_name)
...
class TuGraphDriver:
    ...
    def load_or_create(self, database_name: str):  # pragma: no cover - placeholder
        endpoint = os.environ.get("TUGRAPH_REST_ENDPOINT")
        graph = database_name or os.environ.get("TUGRAPH_REST_GRAPH", "default")
